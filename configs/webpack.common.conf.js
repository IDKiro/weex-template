...
const getEntryFileContent = (source, routerpath, storepath) => {
  ...
  if (hasPluginInstalled) {
    ...
    entryContents = entryContents.replace(/\.\/store/, storepath);
...
const getStoreFileContent = (source) => {
  const dependence = `import Vue from 'vue'\n`;
  let storeContents = fs.readFileSync(source).toString();
  storeContents = dependence + storeContents;
...
const getEntryFile = () => {
  ...
  fs.outputFileSync(entryFile, getEntryFileContent(helper.root(config.entryFilePath), routerFile, storeFile));
  fs.outputFileSync(routerFile, getRouterFileContent(helper.root(config.routerFilePath)));
  fs.outputFileSync(storeFile, getStoreFileContent(helper.root(config.storeFilePath)));
  return {
    index: entryFile
  }
}
